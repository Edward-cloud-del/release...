...
impl ScreenshotCache {
    ...
    fn capture_with_reused_buffer(&mut self, bounds: CaptureBounds) -> Result<String, String> {
        ...
        println!("📐 User selected area: {}x{} at overlay coordinates ({}, {})", 
        ...
        let total_area = match crate::overlay::screen_capture::ScreenCapture::get_total_screen_area() {
            Ok(area) => {
                println!("🖥️ Total screen area detected: {}x{} spanning from ({}, {}) to ({}, {})", 
                         area.width, area.height, area.min_x, area.min_y, area.max_x, area.max_y);
                area
            },
            Err(e) => {
                println!("⚠️ Could not get total screen area: {}, falling back to single screen", e);
        ...
        println!("   Overlay coords: ({}, {}) → Screen coords: ({}, {})", 
        ...
        match screenshots::Screen::all() {
            Ok(screens) => {
                ...
                for (screen_index, screen) in screens.iter().enumerate() {
                    ...
                    println!("   Screen {}: {}x{} at ({}, {}) → bounds ({}, {}) to ({}, {})", 
                             screen_index, display.width, display.height, display.x, display.y,
                             screen_left, screen_top, screen_right, screen_bottom);

                    // Check if the capture area overlaps with this screen
                    let overlaps = screen_x < screen_right && (screen_x + bounds.width as i32) > screen_left &&
                                  screen_y < screen_bottom && (screen_y + bounds.height as i32) > screen_top;

                    if overlaps {
                        println!("✅ Found target screen {}! Capture area overlaps with this screen.", screen_index);
                        ...
                        println!("   Absolute ({}, {}) → Relative ({}, {})", screen_x, screen_y, relative_x, relative_y);
                        
                        // Clamp to screen bounds
                        let safe_x = relative_x.max(0).min(display.width as i32 - bounds.width as i32);
                        let safe_y = relative_y.max(0).min(display.height as i32 - bounds.height as i32);
                        ...
                        println!("   Raw relative: ({}, {}) → Safe: ({}, {})", relative_x, relative_y, safe_x, safe_y);
                        println!("   Requested size: {}x{} → Safe size: {}x{}", bounds.width, bounds.height, safe_width, safe_height);
                        ...
                        println!("📸 Capturing from Screen {} at coordinates ({}, {}) with size {}x{}", 
                            ...
                            Ok(image) => {
                                match image.to_png(None) {
                                    Ok(png_data) => {
                                        ...
                                        println!("   Screen: {} ({}x{} at {})", screen_index, display.width, display.height, display.x);
                                        println!("   Captured: {}x{} at screen coords ({}, {})", safe_width, safe_height, safe_x, safe_y);
                        ...
                        Ok(image) => {
                            match image.to_png(None) {
                                Ok(png_data) => {
                                    ...
                                    println!("📸 Fallback single-screen capture: {}KB", png_data.len() / 1024);
                                    Ok(full_data)
                                },
